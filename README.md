...
Run the setup script. It creates agents **under your Toolhouse account**, deploys them, and creates `app/.env`:
...
This will:
1. Run `th new` for each agent — creating them in **your** account with IDs tied to you
2. Deploy the orchestrator, debater, and synthesizer to Toolhouse
3. Create `app/.env` and `app/.env.example` with your agent URLs

Agent IDs are generated when the script runs — they belong to your account, not the repo maintainer's. Make sure you're authenticated first (step 3).
...
| Path | Purpose |
|------|---------|
| `orchestrator.template.yaml` | Template for orchestrator agent |
| `debater.template.yaml` | Template for debater agent |
| `synthesizer.template.yaml` | Template for synthesizer agent |
| `orchestrator.yaml`, `debater.yaml`, `synthesizer.yaml` | Generated by setup (gitignored) — IDs tied to your account |
...
| `scripts/setup-env.js` | Creates agents, deploys, writes `app/.env` |
...
| `npm run setup:env` | Create agents (your account), deploy, create `app/.env` |
